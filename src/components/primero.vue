...
let guardorespaldo = ref([])



 async function reviso() {

  for (let index = 1; index <= 50; index++) {
      
      await axios.get(`https://pokeapi.co/api/v2/pokemon/${index}`).then((res) => {
      let f = res.data
      todores.value.push(f)


solonombres.value.push({
  nombres: todores.value[index-1].name,
  imagenes: todores.value[index-1].sprites.other["official-artwork"].front_default,
  id: todores.value[index-1].id,
  altura: todores.value[index-1].height,
  peso: todores.value[index-1].weight,
  experiencia: todores.value[index-1].stats[0].base_stat,
  ataque: todores.value[index-1].stats[1].base_stat,
  defensa:  todores.value[index-1].stats[2].base_stat,
  ataqueespecial: todores.value[index-1].stats[3].base_stat,
  defensaespecial: todores.value[index-1].stats[4].base_stat,
  velocidad: todores.value[index-1].stats[5].base_stat,
  tipo: todores.value[index-1].types[0].type.name

})



guardorespaldo.value.push({
  nombres: todores.value[index-1].name,
  imagenes: todores.value[index-1].sprites.other["official-artwork"].front_default,
  id: todores.value[index-1].id,
  altura: todores.value[index-1].height,
...
let pokemonBuscado = ref("")





let buscarPokemon = computed(() => {

  if (pokemonBuscado.value === "") {
    return guardorespaldo.value;
  }

  let nombreBuscado = pokemonBuscado.value.toLowerCase();
  let resultados = solonombres.value.filter(pokemon =>
    pokemon.nombres.toLowerCase().includes(nombreBuscado)
  );
...
function cargonumero(id) {

const found = solonombres.value.find(element => element.id == id);


usodatos.guardoid = id


usodatos.guardoimagenes = found.imagenes

usodatos.guardonombres = found.nombres

usodatos.guardoaltura = found.altura

usodatos.guardopeso = found.peso

usodatos.guardohp = found.experiencia

usodatos.guardoataque = found.ataque

usodatos.guardodefensa = found.defensa

usodatos.guardoataqueespecial = found.ataqueespecial

usodatos.guardodefensaespecial = found.defensaespecial

usodatos.guardovelocidad = found.velocidad

usodatos.guardotipo = found.tipo


}

import {usepokeStore} from "../stores/user.js"
const usodatos = usepokeStore()




let categoriaSeleccionada = ref('todos');

function filtrarPorCategoria() {
  if (categoriaSeleccionada.value === 'todos') {
      guardorespaldo.value = solonombres.value
  } else {
   guardorespaldo.value = solonombres.value.filter(pokemon => pokemon.tipo === categoriaSeleccionada.value);
  }
}


function fuego() {
  categoriaSeleccionada.value = 'fire'
  filtrarPorCategoria()
}
...
function planta() {
  categoriaSeleccionada.value = 'grass'
  filtrarPorCategoria()
}
function agua() {
  categoriaSeleccionada.value = 'water'
  filtrarPorCategoria()
}
function bicho() {
  categoriaSeleccionada.value = 'bug'
  filtrarPorCategoria()
}
function normal() {
  categoriaSeleccionada.value = 'normal'
  filtrarPorCategoria()
}
function electrico() {
  categoriaSeleccionada.value = 'electric'
  filtrarPorCategoria()
}
function veneno() {
  categoriaSeleccionada.value = 'poison'
  filtrarPorCategoria()
}
function tierra() {
  categoriaSeleccionada.value = 'ground'
  filtrarPorCategoria()
}
function hada() {
  categoriaSeleccionada.value = 'fairy'
  filtrarPorCategoria()
}
function dragon() {
  categoriaSeleccionada.value = 'dragon'
  filtrarPorCategoria()
}
function fantasma() {
  categoriaSeleccionada.value = 'ghost'
  filtrarPorCategoria()
}
function hielo() {
  categoriaSeleccionada.value = 'ice'
  filtrarPorCategoria()
}
function psiquico() {
  categoriaSeleccionada.value = 'psychic'
  filtrarPorCategoria()
}
function acero() {
  categoriaSeleccionada.value = 'steel'
  filtrarPorCategoria()
}
function siniestro() {
  categoriaSeleccionada.value = 'dark'
  filtrarPorCategoria()
}
function lucha() {
  categoriaSeleccionada.value = 'fighting'
  filtrarPorCategoria()
}
function volador() {
  categoriaSeleccionada.value = 'flying'
  filtrarPorCategoria()
}
function roca() {
  categoriaSeleccionada.value = 'rock'
  filtrarPorCategoria()
}
...
<nav class="navbar navbar-dark bg-dark fixed-top">
  <div class="container-fluid">
      <img style="width: 200px; height: 80px;" src="../assets/Pokédex_logo.png" alt=""/>
   
    
    <input style="background-color: #535c68; color: white; width: 65%;"   v-model="pokemonBuscado"  class="form-control me-2" type="search" placeholder="Buscador..." aria-label="Search">
    ...
    <div class="offcanvas offcanvas-end text-bg-dark" tabindex="-1" id="offcanvasDarkNavbar" aria-labelledby="offcanvasDarkNavbarLabel">
      <div class="offcanvas-header">
        <h5 class="offcanvas-title" id="offcanvasDarkNavbarLabel">Categorias de pokemon</h5>
      ...
      <div class="offcanvas-body">
        ...
        <button @click="todos()"  value="todos" style="width: 100%;" type="button" class="btn btn-outline-light">Todos</button> <br><br>
        <button @click="planta()" value="grass" style="width: 100%;" type="button" class="btn btn-outline-light">Planta</button> <br><br>
        <button @click="fuego()" value="fire" style="width: 100%;" type="button" class="btn btn-outline-light">Fuego</button> <br><br>
        <button @click="bicho()" value="bicho" style="width: 100%;" type="button" class="btn btn-outline-light">Bicho</button> <br><br>
        <button @click="hada()" value="hada" style="width: 100%;" type="button" class="btn btn-outline-light">Hada</button> <br> <br>
        <button @click="dragon()" value="dragon" style="width: 100%;" type="button" class="btn btn-outline-light">Dragón</button> <br><br>
        <button @click="fantasma()" value="fantasma" style="width: 100%;" type="button" class="btn btn-outline-light">Fantasma</button> <br><br>
        <button @click="tierra()" value="tierra" style="width: 100%;" type="button" class="btn btn-outline-light">Tierra</button> <br><br>
        <button @click="normal()" value="normal" style="width: 100%;" type="button" class="btn btn-outline-light">Normal</button> <br> <br>
        <button @click="psiquico()" value="psiquico" style="width: 100%;" type="button" class="btn btn-outline-light">Psiquico</button> <br><br>
        <button @click="acero()" value="acero" style="width: 100%;" type="button" class="btn btn-outline-light">Acero</button> <br><br>
        <button @click="siniestro()" value="siniestro" style="width: 100%;" type="button" class="btn btn-outline-light">Siniestro</button> <br><br>
        <button @click="electrico()" value="electrico" style="width: 100%;" type="button" class="btn btn-outline-light">Eléctrico</button> <br> <br>
        <button @click="lucha()" value="lucha" style="width: 100%;" type="button" class="btn btn-outline-light">Lucha</button> <br><br>
        <button @click="volador()"  value="volador" style="width: 100%;" type="button" class="btn btn-outline-light">Volador</button> <br><br>
        <button @click="hielo()" value="hielo" style="width: 100%;" type="button" class="btn btn-outline-light">Hielo</button> <br><br>
        <button @click="veneno()" value="veneno" style="width: 100%;" type="button" class="btn btn-outline-light">Veneno</button> <br> <br>
        <button @click="roca()"  value="roca" style="width: 100%;" type="button" class="btn btn-outline-light">Roca</button> <br><br>
        <button @click="agua()" value="agua" style="width: 100%;" type="button" class="btn btn-outline-light">Agua</button> <br><br>
...
<br><br>
      ...
      <div  v-for="(m, i) in buscarPokemon" :key="i" class="card" style="width: 18rem;background-color: transparent;border-radius: 10px;">
        <img
          style="height: 100%"
          :src="m.imagenes"
          alt=""
        />
        <div style="background-color: #535c68; text-align: center; border-radius: 10px;" class="card-body">
          <div >
            <h5 style="color: #fff;text-transform: uppercase; " class="card-title">{{ m.nombres }}</h5>
          </div>
          <p class="card-text">
  ...
  <router-link to="/datospoke">
  <button id="pik" @click="cargonumero(m.id)" >
...
#pik:hover .pword, #pik:hover .pword2 {
  opacity: 1;
}
